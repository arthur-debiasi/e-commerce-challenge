...
export const financeValidation = (costPrice: string, newSalePrice: string) => {
  ...
  const MSG = `O novo preço de venda (${newSalePrice}) precisa ser, no mínimo, o preço de custo de ${costPrice}`
  if (newSalePrice < costPrice) return MSG;
  return OK_MSG;
}
...
export const marketingValidation = (currentSalePrice: string, newSalePrice: string) => {
  ...
  if (!Number(newSalePrice)) return 'Novo Preço de Venda não informado';
  const minSalePrice = (Number(currentSalePrice) * 0.9).toFixed(2);
  const maxSalePrice = (Number(currentSalePrice) * 1.1).toFixed(2);
  const IS_HIGHER_MSG = `O novo preço de venda (${newSalePrice}) precisa ser maior que ${minSalePrice}`;
  const IS_LOWER_MSG = `O novo preço de venda (${newSalePrice}) precisa ser menor que ${maxSalePrice}`
